...
function setDataObject(dataObject) {
	const { nombre, apPaterno, apMaterno, fechaNacimiento, telefono, escuela, correo, contrasena, disciplina } = dataObject;
	return new Promise((res, rej) => {
...
$.ajax(setRequestParams(paramsObj))
	.done((data) => {
			alert({
					tipo: 'success',
					texto: 'Líder F registrado con éxito.',
					onClose: () => { location.href = `${localStorage.baseUrl}/lideres/equipos` }
			});
	})
	.fail((error) => {
			console.log(error)
